...
import { handleCreateShare, handleListShares, handleDeleteShare, handleBatchDeleteShares, handleShareAccess, handleSharePreviewUrl, handleShareDownloadUrl, handleShareListFiles, isTextFile } from './shares';
...
async function checkAuth(request: Request, password?: string, kv?: KVNamespace): Promise<Response | true> {
  ...
  if (kv) {
    // KV-backed: validate UUID token, refresh TTL
    const valid = await validateToken(kv, token);
    return valid ? true : json({ error: 'Unauthorized' }, 401);
  }

  // No KV: stateless fallback — token is the password itself
  return token === password ? true : json({ error: 'Unauthorized' }, 401);
}
    ...
    try {
      ...
      if (path === '/api/login' && request.method === 'POST') {
        ...
        if (providedPw === pw) {
          ...
          // No KV: stateless fallback — use password itself as token
          return json({ token: pw });
        }
      ...
      // GET /api/preview/:key?share_token=... — share preview (public)
      const sharePreviewMatch = path.match(/^\/api\/preview\/(.+)$/);
      if (sharePreviewMatch && request.method === 'GET' && url.searchParams.has('share_token')) {
        if (!env.DB) return json({ error: 'Sharing not configured' }, 503);
        try {
          ...
          return await handleSharePreviewUrl(env, key, shareToken, charset, request);
        } catch { return json({ error: 'Invalid key encoding' }, 400); }
      }

      // GET /api/files/:key?share_token=... — share download (public)
      const shareDownloadMatch = path.match(/^\/api\/files\/(.+)$/);
      if (shareDownloadMatch && request.method === 'GET' && url.searchParams.has('share_token')) {
        if (!env.DB) return json({ error: 'Sharing not configured' }, 503);
        try {
          const key = decodeURIComponent(shareDownloadMatch[1]);
          const shareToken = url.searchParams.get('share_token') || '';
          return await handleShareDownloadUrl(env, key, shareToken, request);
        } catch { return json({ error: 'Invalid key encoding' }, 400); }
      }

      // GET /api/share-files — list files in a share folder (public)
      if (path === '/api/share-files' && request.method === 'GET') {
        if (!env.DB) return json({ error: 'Sharing not configured' }, 503);
        const shareToken = url.searchParams.get('share_token') || '';
        const prefix = url.searchParams.get('prefix') || '';
        return await handleShareListFiles(env, prefix, shareToken, request);
      }
...
export async function handlePreviewUrl(s: StorageConfig, key: string, charset?: string): Promise<Response> {
  ...
  // Only set inline disposition for text files (rendered in iframe).
  // For media/PDF/office, skip to avoid Oracle S3 performance issue with long response-content-disposition.
  const url = await generatePresignedUrl({
    method: 'GET',
    bucket: s.bucket, key,
    region: s.region, accessKeyId: s.accessKeyId, secretAccessKey: s.secretAccessKey,
