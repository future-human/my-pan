...
export function parseListXml(xml: string): { files: Array<{ key: string; size: number; lastModified: string }>; commonPrefixes: string[]; isTruncated: boolean; nextMarker: string | null } {
  ...
  for (const [, block] of xml.matchAll(/<CommonPrefixes>([\s\S]*?)<\/CommonPrefixes>/g)) {
    const prefix = decodeXml((block.match(/<Prefix>([\s\S]*?)<\/Prefix>/) || [])[1] || '');
  ...
  return { files, commonPrefixes, isTruncated, nextMarker: nextMarker ? decodeXml(nextMarker) : null };
}
