...
app.get('/api/files/:key(*)', async (req, res, next) => {
  if (!req.query.share_token) return next();
  ...
  try {
    ...
    const r = adaptRequest(req);
    const response = await handleShareDownloadUrl(env, key, String(req.query.share_token || ''), r as unknown as Request);
    await sendResponse(res, response);
  } catch {
...
// Share preview (public, ?share_token=xxx)
app.get('/api/preview/:key(*)', async (req, res, next) => {
  ...
  try {
    ...
    if (err) return res.status(400).json({ error: err });
    const r = adaptRequest(req);
    const response = await handleSharePreviewUrl(env, key, String(req.query.share_token || ''), req.query.charset ? String(req.query.charset) : undefined, r as unknown as Request);
    await sendResponse(res, response);
  } catch {
    res.status(400).json({ error: 'Invalid key encoding' });
  }
