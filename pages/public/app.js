...
async function init() {
  // 分享模式：跳过登录，直接加载分享文件
  if (window.SHARE_CONFIG) {
    document.getElementById('login-overlay').style.display = 'none';
    currentPrefix = window.SHARE_CONFIG.basePrefix || '';
    await loadShareFiles();
    return;
  }

  // 从登录页切换语言 → 跳过鉴权探测，直接显示登录表单
  if (sessionStorage.getItem('my-pan_on_login') === '1') {
    sessionStorage.removeItem('my-pan_on_login');
    showLogin();
  ...
  if (savedToken) {
    ...
    // Token 失效 → 清除，直接进入登录（跳过无密码探测，因为曾经登过说明服务端需要鉴权）
  ...
  // 无保存 token → 尝试无密码模式（服务端未配置密码时直接进入）
  const ok = await tryAuth();
  if (ok) {
...
async function loadStorages() {
  ...
  const switchStorage = async (id) => {
    ...
    clearSelection(); currentPrefix = ''; searchQuery = '';
...
function showLogin() {
  ...
  sessionStorage.setItem('my-pan_on_login', '1');
}
...
async function loadShareFiles() {
  ...
  try {
    const resp = await fetch(apiBase + '/api/share-files?' +
      'share_token=' + encodeURIComponent(cfg.shareToken) +
      '&prefix=' + encodeURIComponent(cfg.basePrefix));
    if (!resp.ok) {
    ...
    console.error('[my-pan] loadShareFiles: 加载失败', err);
    const tbody = document.getElementById('file-tbody');
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:40px;color:#e84545">' + err.message + '</td></tr>';
    toast(t('加载文件列表失败') + ': ' + err.message, 'error');
  }
}

function renderFiles() {
  const isShare = !!window.SHARE_CONFIG;
  document.getElementById('skeleton-loading').style.display = 'none';
  const tbody = document.getElementById('file-tbody');
  ...
  if (searchQuery) {
    ...
    for (const f of matched) {
      html += `
    <tr draggable="true" data-item-type="file" data-item-key="${escAttr(f.key)}">
      ...
      <td class="actions">
        ...
        ${isShare ? '' : `<button class="rename" data-key="${escAttr(f.key)}">${t('重命名')}</button>`}
        ${isShare ? '' : `<button class="delete" data-key="${escAttr(f.key)}">${t('删除')}</button>`}
        ${!isShare && sharingEnabled ? `<button class="share" data-key="${escAttr(f.key)}">${t('分享')}</button>` : ''}
  ...
  // Share root empty state
  if (totalEntries === 0 && isShare && currentPrefix === window.SHARE_CONFIG.basePrefix) {
    tbody.innerHTML = '';
    empty.style.display = 'block';
    document.getElementById('empty-text').textContent = t('此文件夹为空');
    clearSelection();
    setupColumnResize();
    setupDragAndDrop();
    return;
  }
  if (currentPrefix && !(isShare && currentPrefix === window.SHARE_CONFIG.basePrefix)) {
  ...
  for (const fd of folderData) {
    html += `
    <tr class="folder-row" draggable="true" data-item-type="folder" data-item-key="${escAttr(fd.prefix)}" data-folder="${escAttr(fd.folder)}" data-prefix="${escAttr(fd.prefix)}">
      ...
      <td class="actions">
        ...
        ${isShare ? '' : `<button class="rename" data-prefix="${escAttr(fd.prefix)}">${t('重命名')}</button>`}
        ${isShare ? '' : `<button class="delete" data-prefix="${escAttr(fd.prefix)}">${t('删除')}</button>`}
        ${!isShare && sharingEnabled ? `<button class="share" data-prefix="${escAttr(fd.prefix)}">${t('分享')}</button>` : ''}
        <button class="preview btn-hidden" tabindex="-1">${t('预览')}</button>
      </td>
    </tr>`;
  }
  ...
  for (const f of sorted) {
    ...
    html += `
    <tr draggable="true" data-item-type="file" data-item-key="${escAttr(fullKey)}">
      ...
      <td class="file-date">${formatDate(f.lastModified)}</td>
      <td class="actions">
        <button class="download" data-key="${escAttr(fullKey)}">${t('下载')}</button>
        ${isShare ? '' : `<button class="rename" data-key="${escAttr(fullKey)}">${t('重命名')}</button>`}
        ${isShare ? '' : `<button class="delete" data-key="${escAttr(fullKey)}">${t('删除')}</button>`}
        ${!isShare && sharingEnabled ? `<button class="share" data-key="${escAttr(fullKey)}">${t('分享')}</button>` : ''}
        <button class="preview${isPreviewable(fullKey) ? '' : ' btn-hidden'}" data-key="${escAttr(fullKey)}">${t('预览')}</button>
      </td>
    </tr>`;
  }
  tbody.innerHTML = html;
  ...
  // Folder row click with Ctrl/Shift multi-select (except name & buttons)
  tbody.querySelectorAll('.folder-row').forEach(tr => {
    tr.addEventListener('click', (e) => {
      ...
      if (!cb) return;
      const key = tr.dataset.prefix;
      // Click on checkbox column td (padding around checkbox) → toggle without clearing
      const clickedTd = e.target.closest('td');
      const isCheckboxCol = clickedTd && !clickedTd.querySelector('button') && clickedTd.querySelector('input[type=checkbox]');
      if (!e.shiftKey && !e.ctrlKey && !e.metaKey && isCheckboxCol) {
        cb.checked = !cb.checked;
        cb.dispatchEvent(new Event('change'));
        lastClickedKey = key;
        return;
      }
      if (e.shiftKey && lastClickedKey !== null) {
        selectItemRange(lastClickedKey, key);
        lastClickedKey = key;
...
function bindFileRowEvents() {
  ...
  // File row click with Ctrl/Shift multi-select
  ...
  tbody.querySelectorAll('tr:not(.folder-row) button.preview').forEach(btn => {
    btn.addEventListener('click', () => { document.getElementById('preview-charset').value = 'utf-8'; previewFile(btn.dataset.key); });
  });
...
function updateBatchToolbar() {
  ...
  const someChecked = itemCount > 0 && !allChecked;
...
async function downloadFile(key) {
  try {
    const cfg = window.SHARE_CONFIG;
    const url = cfg
      ? apiBase + '/api/files/' + encodeURIComponent(key) + '?share_token=' + encodeURIComponent(cfg.shareToken)
      : fileApi('/api/files/' + encodeURIComponent(key));
    const headers = cfg ? {} : { 'X-Auth-Token': token };
    const resp = await fetch(url, { headers });
    if (!resp.ok) throw new Error('获取下载链接失败: ' + resp.status);
    const { url: dlUrl } = await resp.json();
...
async function previewFile(key) {
  try {
    ...
    const cfg = window.SHARE_CONFIG;
    const url = cfg
      ? apiBase + '/api/preview/' + encodeURIComponent(key) + '?share_token=' + encodeURIComponent(cfg.shareToken) + '&charset=' + charset
      : fileApi('/api/preview/' + encodeURIComponent(key) + '?charset=' + charset);
    const headers = cfg ? {} : { 'X-Auth-Token': token };
    const resp = await fetch(url, { headers });
    if (!resp.ok) throw new Error('获取预览链接失败: ' + resp.status);
    const { url: pvUrl, text } = await resp.json();
    showPreview(key, pvUrl, text);
  } catch (err) {
...
function showPreview(key, url, isText) {
  previewKey = key;
  const ptype = getPreviewType(key);
  ...
  document.getElementById('preview-office').style.display = 'none';
...
function closePreview() {
  ...
  document.getElementById('preview-office').style.display = 'none';
  document.getElementById('preview-office').innerHTML = '';
}
...
async function renderPdf(url, key) {
  const container = document.getElementById('preview-pdf');
  container.style.display = '';
  container.innerHTML = '<div class="preview-loading">' + t('加载中...') + '</div>';
  try {
    if (typeof pdfjsLib === 'undefined') throw new Error(t('PDF 预览库未加载'));
    console.time('[my-pan] PDF fetch');
    const arrayBuffer = await xhrGetBuffer(url);
    console.timeEnd('[my-pan] PDF fetch');
...
async function renderOffice(url, key) {
  const container = document.getElementById('preview-office');
  container.style.display = '';
  container.innerHTML = '<div class="preview-loading">' + t('加载中...') + '</div>';
  try {
    const ext = (key.split('.').pop() || '').toLowerCase();
    console.time('[my-pan] Office fetch');
    const arrayBuffer = await xhrGetBuffer(url);
    console.timeEnd('[my-pan] Office fetch');
...
document.getElementById('preview-iframe').addEventListener('load', () => {
  document.getElementById('preview-loading').style.display = 'none';
  document.getElementById('preview-iframe').style.display = '';
});
...
// ===== Lang toggle =====
// Button #lang-btn is auto-initialized by i18n.js
...
document.getElementById('logout-btn').addEventListener('click', async () => {
  ...
  files = [];
  currentPrefix = '';
  selectedKeys.clear(); selectedFolders.clear(); lastClickedKey = null;
  showLogin();
  const loginBtn = document.getElementById('login-btn');
  loginBtn.disabled = false;
  loginBtn.textContent = t('进入');
  document.getElementById('login-error').style.display = 'none';
  document.getElementById('password-input').value = '';
...
function syncFolderCheckboxes(checkboxes) {
  checkboxes.forEach(cb => {
    ...
    if (filesUnder.length > 0) {
      const selectedCount = filesUnder.filter(f => selectedKeys.has(f.key)).length;
      cb.checked = selectedCount === filesUnder.length;
      cb.indeterminate = selectedCount > 0 && selectedCount < filesUnder.length;
      // Keep selectedFolders in sync with file selection state so visual
      // .checked state never contradicts internal state, which would cause
      // Ctrl+click to toggle the checkbox OFF and wipe all selections.
      if (selectedCount === filesUnder.length && selectedCount > 0) {
        selectedFolders.add(prefix);
      } else {
        selectedFolders.delete(prefix);
      }
    } else {
      cb.checked = selectedFolders.has(prefix);
...
function clearSelection() {
  ...
  selectedFolders.clear();
  lastClickedKey = null;
  document.querySelectorAll('input.file-checkbox, input.folder-checkbox').forEach(cb => {
    cb.checked = false;
  });
...
function getVisibleItems() {
  const items = [];
  const tbody = document.getElementById('file-tbody');
  tbody.querySelectorAll('tr[data-item-key]').forEach(tr => {
    items.push({
      type: tr.dataset.itemType,
      key: tr.dataset.itemKey
    });
  });
  return items;
}

function selectItemRange(fromKey, toKey) {
  const items = getVisibleItems();
  const fromIdx = items.findIndex(item => item.key === fromKey);
  const toIdx = items.findIndex(item => item.key === toKey);
  ...
  for (let i = start; i <= end; i++) {
    ...
    if (item.type === 'file') {
      ...
      selectedFolders.add(item.key);
      for (const f of files) {
        if (!f.key.startsWith(item.key)) continue;
        if (f.key.endsWith('/')) {
          selectedFolders.add(f.key);
        } else {
  ...
  document.querySelectorAll('input.folder-checkbox').forEach(cb => {
    cb.checked = selectedFolders.has(cb.dataset.prefix);
  });
  const sa = document.getElementById('select-all');
  sa.checked = false;
  sa.indeterminate = false;
...
function setupDragAndDrop() {
  ...
  tbody.addEventListener('dragstart', (e) => {
    ...
    const isSelected = itemType === 'file'
      ? selectedKeys.has(itemKey)
      : selectedFolders.has(itemKey);
    ...
    const moveFiles = [...selectedKeys].filter(k =>
      ![...selectedFolders].some(p => k.startsWith(p))
    );
    const moveFolders = [...selectedFolders].filter(p =>
      ![...selectedFolders].some(other => other !== p && p.startsWith(other))
    );

    e.dataTransfer.setData('application/x-mypan-move', JSON.stringify({
      files: moveFiles,
      folders: moveFolders
    }));
    ...
    tbody.querySelectorAll('tr[data-item-key]').forEach(row => {
      const k = row.dataset.itemKey;
      const t = row.dataset.itemType;
      if ((t === 'file' && selectedKeys.has(k)) || (t === 'folder' && selectedFolders.has(k))) {
  ...
  tbody.addEventListener('dragend', () => {
    tbody.querySelectorAll('tr.drag-source').forEach(row => row.classList.remove('drag-source'));
    if (dragOverFolder) {
      dragOverFolder.classList.remove('drag-over-folder');
      dragOverFolder = null;
    }
  });

  tbody.addEventListener('dragover', (e) => {
    const tr = e.target.closest('tr.folder-row[data-item-key]');
    if (dragOverFolder && dragOverFolder !== tr) {
      dragOverFolder.classList.remove('drag-over-folder');
    }
    if (tr) {
      e.preventDefault();
      e.dataTransfer.dropEffect = 'move';
      if (dragOverFolder !== tr) {
        dragOverFolder = tr;
        tr.classList.add('drag-over-folder');
      }
    } else {
      dragOverFolder = null;
    }
  ...
  tbody.addEventListener('drop', async (e) => {
    ...
    if (!tr) return;
    e.preventDefault();
    tr.classList.remove('drag-over-folder');
    dragOverFolder = null;

    const targetPrefix = tr.dataset.itemKey;
    const raw = e.dataTransfer.getData('application/x-mypan-move');
    if (!raw) return;
    ...
    await moveItems(data.files || [], data.folders || [], targetPrefix);
  });
}

async function moveItems(sourceFiles, sourceFolders, targetPrefix) {
  const toMove = [];

  for (const key of sourceFiles) {
    const name = key.substring(key.lastIndexOf('/') + 1);
    const destKey = targetPrefix + name;
    if (key === destKey) continue;
    if (files.some(f => f.key === destKey)) continue;
    toMove.push({ sourceKey: key, destKey });
  }

  for (const prefix of sourceFolders) {
    if (prefix === targetPrefix) continue;
    if (targetPrefix.startsWith(prefix)) continue;

    const folderName = prefix.substring(0, prefix.length - 1).split('/').pop();
    const items = files.filter(f => f.key.startsWith(prefix));
    for (const f of items) {
      const relPath = f.key.slice(prefix.length);
      const destKey = targetPrefix + folderName + '/' + relPath;
      if (f.key === destKey) continue;
      if (files.some(ex => ex.key === destKey)) continue;
      toMove.push({ sourceKey: f.key, destKey });
    }
  }
  ...
  await runPool(toMove.map(m => async () => {
    try {
      const resp = await fetch(fileApi('/api/rename'), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'X-Auth-Token': token },
        body: JSON.stringify({ sourceKey: m.sourceKey, destinationKey: m.destKey }),
      });
      if (!resp.ok) throw new Error((await resp.json().catch(() => ({}))).error || `HTTP ${resp.status}`);
    } catch (err) {
  ...
  const movedKeys = new Set(toMove.map(m => m.sourceKey));
  for (const prefix of sourceFolders) {
    let p = prefix;
    while (p) {
      const hasOther = files.some(f =>
        f.key.startsWith(p) && !movedKeys.has(f.key) && f.key !== p
      );
      if (!hasOther) {
        await deleteFileSilent(p);
      }
...
function updateBreadcrumb() {
  ...
  const cfg = window.SHARE_CONFIG;
  if (cfg) {
    // Share mode: root is the shared folder name, not the bucket root
    const baseLen = cfg.basePrefix.length;
    let html = `<span class="breadcrumb-item" data-prefix="${escAttr(cfg.basePrefix)}">${esc(cfg.folderName)}</span>`;
    let path = cfg.basePrefix;
    const rest = currentPrefix.slice(baseLen);
    if (rest) {
      const parts = rest.split('/').filter(Boolean);
      for (let i = 0; i < parts.length; i++) {
        path += parts[i] + '/';
        html += '<span class="breadcrumb-sep">/</span>';
        html += `<span class="breadcrumb-item" data-prefix="${escAttr(path)}">${esc(parts[i])}</span>`;
      }
    }
    bc.innerHTML = html;
  } else {
...
// Mirrors worker/src/shares.ts TEXT_EXTS + MEDIA_EXTS — keep in sync
...
// Mirrors worker/src/shares.ts getFileIcon — keep in sync
...
contextMenu.querySelectorAll('.context-item').forEach(item => {
  item.addEventListener('click', () => {
    ...
    if (action === 'preview') { document.getElementById('preview-charset').value = 'utf-8'; previewFile(key); }
