...
async function renderPdf(url, key) {
  ...
  try {
    if (typeof pdfjsLib === 'undefined') throw new Error('PDF 预览库未加载');
    ...
    container.innerHTML = '<div class="preview-error">PDF 加载失败: ' + esc(err.message) + '</div>';
  }
}
...
async function renderOffice(url, key) {
  ...
  try {
    ...
    if (ext === 'docx') {
      ...
      container.innerHTML = '<div class="preview-error">PPTX 预览暂不支持，请下载后查看</div>';
    } else {
      container.innerHTML = '<div class="preview-error">不支持的文档格式</div>';
    }
  } catch (err) {
    console.error('[my-pan] renderOffice:', err);
    container.innerHTML = '<div class="preview-error">文档加载失败: ' + esc(err.message) + '</div>';
  }
}
...
(function () {
  ...
  langBtn.innerHTML = isZh
    ? `<svg class="lang-icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
  <path fill="#1677FF" d="M310.656 64h-64.64l0.128 91.008H64v292.224h182.144l-0.064 136.64h64.576V447.296H492.8V154.88l-182.08 0.064V64zM246.144 219.52v163.2H128.512V219.52h117.632z m182.08 0v163.2H310.656V219.52h117.568z"/>
  <path fill="#8C8C8C" d="M770.24 492.224h-73.024L507.52 959.744h69.376l51.392-121.408 3.328-8.32h204.16l3.328 8.32 51.264 121.408H960L770.24 492.16z m-36.48 82.432l12.288 30.784 56.704 141.824 7.232 18.176H657.28l7.296-18.176 56.832-141.824 12.352-30.72z"/>
  <path fill="#8C8C8C" d="M174.08 733.76l-0.064-77.824H109.44v77.824a168.832 168.832 0 0 0 157.76 168.512l11.072 0.32h123.328v-64.576H277.952l-7.36-0.192a104.384 104.384 0 0 1-96.512-104.064z m436.288-559.68h123.392c57.6 0 104.32 46.72 104.32 104.32v77.824h64.448l0.064-77.824a168.832 168.832 0 0 0-157.76-168.448l-11.072-0.384H610.368V174.08z"/>
</svg>`
    : `<svg class="lang-icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
  <path fill="#8C8C8C" d="M310.656 64h-64.64l0.128 91.008H64v292.224h182.144l-0.064 136.64h64.576V447.296H492.8V154.88l-182.08 0.064V64zM246.144 219.52v163.2H128.512V219.52h117.632z m182.08 0v163.2H310.656V219.52h117.568z"/>
  <path fill="#1677FF" d="M770.24 492.224h-73.024L507.52 959.744h69.376l51.392-121.408 3.328-8.32h204.16l3.328 8.32 51.264 121.408H960L770.24 492.16z m-36.48 82.432l12.288 30.784 56.704 141.824 7.232 18.176H657.28l7.296-18.176 56.832-141.824 12.352-30.72z"/>
  <path fill="#8C8C8C" d="M174.08 733.76l-0.064-77.824H109.44v77.824a168.832 168.832 0 0 0 157.76 168.512l11.072 0.32h123.328v-64.576H277.952l-7.36-0.192a104.384 104.384 0 0 1-96.512-104.064z m436.288-559.68h123.392c57.6 0 104.32 46.72 104.32 104.32v77.824h64.448l0.064-77.824a168.832 168.832 0 0 0-157.76-168.448l-11.072-0.384H610.368V174.08z"/>
</svg>`;
})();
...
async function moveItems(sourceFiles, sourceFolders, targetPrefix) {
  ...
  for (const prefix of sourceFolders) {
    ...
    while (p) {
      ...
      if (!hasOther) {
        try { await deleteFileSilent(p); } catch { /* ignore */ }
      }
